...
`enclave-server/` is a reusable Rust template: a fresh secp256k1 keypair every boot (so on-chain verification is a plain `ecrecover`, not hand-rolled Ed25519), an outbound-network allowlist, a health-check endpoint, and attestation-gated KMS secrets, where a secret is only ever decrypted from inside an enclave presenting a matching attestation, never visible in transit or to whoever holds the cloud credentials. Apps are drop-in modules under `examples/<name>/`, wired in by a one-line include; app code never lives inside the server template itself.
